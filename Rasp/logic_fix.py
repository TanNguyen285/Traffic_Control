...
class TrafficLogic:
    def __init__(self, yolo_ai, cnn_service, pre_proc, uart, cam, eth_service, station_id='TRAM_A'):
        ...
        self.bien_run1       = False   # True khi nhận "run1" → thoát kẹt tức thời
    ...
    def thuc_thi_AI(self):
        ...
        if self.operation_mode == "single":
            ...
            if self.ket_local:
                ...
                while True:
                    # ── run1: thoát kẹt tức thời → gửi m2, dừng ─────────────
                    if self.bien_run1:
                        self.bien_run1 = False
                        self._last_uart_cmd = None
                        self._send_uart_single_shot("m2")
                        print(f"[{self.id}] run1 → thoát kẹt khẩn cấp, gửi m2")
        ...
        if cmd_logic in ("A", "B"):
            ...
            while True:
                ...
                if self.bien_run1:
                    ...
                    self._send_uart_single_shot("m2")
                    print(f"[{self.id}] run1 → thoát kẹt khẩn cấp, gửi m2")
                    return self.result_AI(conn, k_rem, x_rem, "m2", is_emergency=True), "m2"
    ...
    def result_AI(self, remote_connected, ket_remote, xe_remote, cmd, is_emergency=False):
