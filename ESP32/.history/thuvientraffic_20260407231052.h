...
public:
    ...
    uint32_t bangtimer(uint8_t mode) {
        const uint32_t bangmode[] = {10, 15, 20, 25, 30}; // Giây (m0/,m1,m2,m3,m4)
        return (mode <= 4) ? bangmode[mode] : bangmode[0];
    }
    ...
    void chayTuDong(uint8_t mode) {
        ...
        uint32_t thoiGianToanPha = bangtimer(mode) * 1000; // Đổi sang ms
