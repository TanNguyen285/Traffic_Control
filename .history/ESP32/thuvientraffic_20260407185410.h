...
// ============================================================
// 1. DIEU_KHIEN_PIN: Quản lý các chân vật lý
// ============================================================
class DieuKhienPin {
public:
    DieuKhienPin(uint8_t pinXanh, uint8_t pinDo, uint8_t pinVang)
        : _pXanh(pinXanh), _pDo(pinDo), _pVang(pinVang) {}

    void batDau() {
        pinMode(_pXanh, OUTPUT);
        pinMode(_pDo,   OUTPUT);
        pinMode(_pVang, OUTPUT);
        tatHet();
    }

    void denXanh(bool on) { digitalWrite(_pXanh, on ? HIGH : LOW); }
    void denDo(bool on)   { digitalWrite(_pDo,   on ? HIGH : LOW); }
    void denVang(bool on) { digitalWrite(_pVang, on ? HIGH : LOW); }
...
public:
    ...
    uint32_t layThoiGianMode(uint8_t mode) {
        const uint32_t bangTra[] = {10, 15, 20, 25, 30}; // Giây
        return (mode <= 4) ? bangTra[mode] : bangTra[0];
    }

    void chayTuDong(uint8_t mode) {
        if (!_dangKichHoat) return;

        uint32_t hienTai = millis();
        uint32_t thoiGianToanPha = layThoiGianMode(mode) * 1000; // Đổi sang ms
        uint32_t daTroiQua = hienTai - _tgBatDau;

        // Logic đèn vàng (bảng báo): Sáng 5s cuối trước khi đổi pha
        if (daTroiQua >= (thoiGianToanPha - 5000)) {
            _pin.denVang(1);
        } else {
            _pin.denVang(0);
        }
