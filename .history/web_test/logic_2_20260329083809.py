...
class TrafficLogic:
    ...
    def AI_CNN_SCI(self, selected_image=None):
        ...
        if ket_local:
            ...
            yolo_res, xe_local = self.ai.detect(yolo_image, brightness)
    ...
    # Gửi ảnh gốc vào đây (JS sẽ đưa vào ô 'anhgoc')
    "input_image": self._to_base64_url(frame_raw), 
    
    # Gửi ảnh đã vẽ khung vào đây (JS sẽ đưa vào ô 'sauxuly')
    "yolo_image": self._to_base64_url(frame_yolo) 
}
        self.uart.send(cmd)
        return result, cmd
