...
class TrafficLogic:
    def __init__(self, yolo_ai, cnn_service, pre_proc, uart, cam, net_service, station_id='A',
        ...
        self.uart = uart
        self.cam = cam
        self.net = net_service # Truyền class TrafficNetwork vào đây
        
        self.id = station_id.upper()
        self.t_cho = 0 
        self.t_modes = {'m1': t_m1, 'm2': t_m2, 'm3': t_m3, 'm4': t_m4, 'm5': t_ket}
        self.t_y = t_y
    ...
    def AI_CNN_SCI(self, selected_image=None):
        ...
        # --- BƯỚC MỚI: Giao tiếp Ethernet ---
        # Gửi dữ liệu của mình đi
        self.net.send_status(is_jam_local, xe_local)
        # Nhận dữ liệu trạm đối diện
        remote_data = self.net.get_remote_data()

        # 3. Phân biệt A/B dựa trên ID trạm
        if self.id == 'A':
        ...
        # 4. Logic điều khiển (giữ nguyên như bản cũ của bạn)
        cmd = "m1"
        t_m = self.t_modes['m1']
        ...
        return {
            ...
            "xe_tai_cho": xe_local,
            "xe_doi_dien": xe_b if self.id == 'A' else xe_a,
            "tram_doi_dien_ket": B if self.id == 'A' else A,
