...
class TrafficLogic:
    ...
    def AI_CNN_SCI(self, selected_image=None):
        ...
        if self.id == 'A':
            ...
            b = not B
            mode_local, xe_b = xe_local, remote_data.get('xe', 0)
        else:
            B, b = ket_local, not ket_local
            A = remote_data.get('ket', False)
            a = not A
            xe_b, mode_local = xe_local, remote_data.get('xe', 0)
    ...
    "xe_local": xe_local,
    "xe_remote": xe_b if self.id == 'A' else mode_local,#trạng thái kẹt xe gửi đi
