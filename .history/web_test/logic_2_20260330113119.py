...
class TrafficLogic:
    def __init__(self, yolo_ai, cnn_service, pre_proc, uart, cam, eth_service, station_id='A',
        ...
        self.bie_run_run = False

    # 🔥 CALLBACK từ UART
    def uart_esp32_rasp(self):
        print("[AI] Nhận run từ UART")
        self.bie_run_run = True
        

    def AI_CNN_SCI(self, selected_image=None):
        if not self.bie_run_run:
            return {"error": "Chưa nhận lệnh run từ ESP32"}, "m0"
        seft.bie_run_run = False # Reset trigger để chờ lần sau
