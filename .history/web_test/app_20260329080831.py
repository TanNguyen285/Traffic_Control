...
# ==========================================================
# 6. LUỒNG XỬ LÝ AI CHỦ ĐỘNG (BACKGROUND THREAD)
# ==========================================================
import threading
import time
...
def ai_main_loop():
    """
    Luồng này chạy độc lập, liên tục quét AI và đếm giây.
    Nó sẽ giúp Terminal nhảy số liên tục.
    """
    print("\n[HỆ THỐNG] Đã kích hoạt luồng AI chủ động...")
    while True:
        try:
            # Gọi engine xử lý (Timer bên trong logic_2 sẽ tự chạy)
            # Truyền selected_image nếu có người vừa upload ảnh từ web
            global selected_image
            res, cmd = engine.AI_CNN_SCI(selected_image)
            
            if selected_image is not None:
                selected_image = None # Reset sau khi xử lý xong ảnh upload
            
            # Tốc độ quét: 0.5 giây/lần (đủ để đếm giây mượt mà)
            time.sleep(0.5) 
...
# Khởi tạo luồng ngầm
t = threading.Thread(target=ai_main_loop, daemon=True)
t.start()

# ==========================================================
# 7. QUẢN LÝ ĐÓNG HỆ THỐNG
# ==========================================================
import atexit
atexit.register(lambda: cam.stop())

if __name__ == "__main__":
    # Tắt use_reloader=False để tránh việc Flask chạy 2 luồng AI cùng lúc
