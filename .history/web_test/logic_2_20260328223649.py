...
class TrafficLogic:
    ...
    def AI_CNN_SCI(self, selected_image=None):
        ...
        # Thay vì in toàn bộ result, chúng ta chỉ in những thứ cần debug
        print(f"\n[LOGIC] --- KẾT QUẢ PHÂN TÍCH ---")
        print(f"  + Trạng thái CNN: {result['cnn_status']}")
        print(f"  + Số xe tại chỗ: {result['xe_local']}")
        print(f"  + Độ sáng: {result['brightness']}")
        print(f"  + Lệnh sẽ gửi: {cmd}") 
        print(f"---------------------------------\n")
