...
class GLKA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.reparam_conv is not None:
            branch_main = self.reparam_conv(anchor) #chỉ dùng conv đã gộp sau khi deploy
        else:
            branch_main = self.branch1(anchor) + self.branch2(anchor) + \
                          self.branch3(anchor) + self.branch4(anchor)
